...
public final class J {
    ...
    private static void recalcRev(Node n) {
        n.isReversed = (n.isReversed != true);
    }

    private static void push(Node n) {
        if (n != null && n.isReversed) {
            n.isReversed = false;
            Node tmp = n.left;
            n.left = n.right;
            n.right = tmp;
            if (n.left != null) recalcRev(n.left); 
            if (n.right != null) recalcRev(n.right); 
            // recalcCnt(n);
            // recalcCnt(n.right);
            // recalcCnt(n.left);
        }
   ...
   private static void dfsDebug(Node tree)
    ...
    private static Node reverseRange(Node tree, int a, int b) {
        Node[] nodes = new Node[2];
        split(tree, a, nodes);
        Node[] sp = new Node[2];
        split(nodes[1], b - a + 1, sp);
        recalcRev(sp[0]);
        return merge(merge(nodes[0], sp[0]), sp[1]);
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            tree = insert(tree, new Node(i+1), i+1);
        }
        for (int i = 0; i < r; i++) {
            tree = reverseRange(tree, in.nextInt()-1, in.nextInt()-1);
        }
        dfs(tree);
        in.close();
    }
