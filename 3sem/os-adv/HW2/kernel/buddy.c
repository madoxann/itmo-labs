...
int get_blk_xor(int ind) { return ind / 2; }
...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  xor_blk:");
    bd_print_vector(bd_sizes[k].xor_blk, NBLK(k));
    if (k > 0) {
      printf("  split:");
...
void *bd_malloc(uint64 nbytes) {
  ...
  bit_flip(bd_sizes[k].xor_blk, get_blk_xor(blk_index(k, p)));
  for (; k > fk; k--) {
    // split a block at size k and mark one half allocated at size k-1
    // and put the buddy on the free list at size k-1
    char *q = p + BLK_SIZE(k - 1);  // p's buddy
    bit_set(bd_sizes[k].split, blk_index(k, p));
    bit_flip(bd_sizes[k - 1].xor_blk, get_blk_xor(blk_index(k - 1, p)));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_set(bd_sizes[k].xor_blk, get_blk_xor(bi));  // free p at size k

    if (bit_isset(bd_sizes[k].xor_blk,
                  get_blk_xor(bi))) {  // is buddy allocated?
...
int bd_mark(void *start, void *stop, int is_unavalible) {
  int bi, bj, marked = 0;
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    if (k < nsizes - 2) {
      if (is_unavalible && (bi % 2)) {
        lst_push(&bd_sizes[k].free, addr(k, bi - 1));
        marked += BLK_SIZE(k);
      }
      if (!is_unavalible && (bj % 2)) {
    ...
    for (; bi < bj; bi++) {
      ...
      if (k > 0) bit_set(bd_sizes[k].split, bi);

      bit_flip(bd_sizes[k].xor_blk, get_blk_xor(bi));
    }
  }
...
int bd_mark_data_structures(char *p, int *f_ptr) {
  int meta = p - (char *)bd_base;
  printf("bd: %d meta bytes for managing %d bytes of memory\n", meta,
         BLK_SIZE(MAXSIZE));
  *f_ptr += bd_mark(bd_base, p, 0);
  return meta;
}

// Mark the range [end, HEAPSIZE) as allocated
int bd_mark_unavailable(void *end, void *left, int *f_ptr) {
  int unavailable = BLK_SIZE(MAXSIZE) - (end - bd_base);
  if (unavailable > 0) unavailable = ROUNDUP(unavailable, LEAF_SIZE);
  printf("bd: 0x%x bytes unavailable\n", unavailable);

  void *bd_end = bd_base + BLK_SIZE(MAXSIZE) - unavailable;
  *f_ptr += bd_mark(bd_end, bd_base + BLK_SIZE(MAXSIZE), 1);
  return unavailable;
}
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    bd_sizes[k].xor_blk = p;
    memset(bd_sizes[k].xor_blk, 0, sz / 2);
    p += sz / 2;
  }
  ...
  int meta = bd_mark_data_structures(p, free);

  // mark the unavailable memory range [end, HEAP_SIZE) as allocated,
  // so that buddy will not hand out that memory.
  int unavailable = bd_mark_unavailable(end, p, free);
