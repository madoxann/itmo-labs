...
void dump(void) {
  struct proc get_register;
  get_register = *myproc();

  int cnt = 2;
  for (uint64 *i = &get_register.trapframe->s2; i < &get_register.trapframe->t3;
       i++) {
    printf("s%d %d\n", cnt++, *(uint32 *)i);
  }
}
...
int dump2(int pid, int register_num, uint64 *return_value) {
  ...
  struct proc *p;
  struct proc *called_from = myproc();

  for (p = proc; p < &proc[NPROC]; p++) {
    if (p->pid == pid) {
      if (pid != called_from->pid && p->parent->pid != called_from->pid)
        return -1;

      return (copyout(called_from->pagetable, (uint64)return_value,
